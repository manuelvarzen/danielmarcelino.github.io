...
###Aula 4: limpeza de dados, laços (loops), aplicando funções
...
- For/while laços para repetição de tarefas
- `apply()`, `lapply()`, `sapply()`, `tapply()`
- `with()` para especificar escopo
...
- For a simple example, I've corrupted the `TVhours` column in the survey data
...
```{r}
as.numeric(as.factor(3.1))
as.numeric(as.character(as.factor(3.1)))
```
...
tv.hours.strings
# Use gsub() to replace everything but digits and '.' with a blank ""
...
sapply(survey, is.factor) # Returns a vector with named elements
```
...
tapply(rep(1, nrow(Cars93)), INDEX = Cars93[c("Origin", "DriveTrain")], FUN=sum) # This one may take a moment to figure out
```
...
- **Homework 2** has been posted to the course website
  - **Due: Thursday, November 6, 4:30pm**
  - Submit your .Rmd file on Blackboard
  - Course website contains grading rubric

- **Lab 4** has been posted to the course website
