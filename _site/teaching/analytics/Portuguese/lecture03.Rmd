...
```{r}
Cars93.metric <- transform(Cars93, KMPL.city = 0.425 * MPG.city, KMPL.highway = 0.425 * MPG.highway)
...
Cars93.metric$KMPL.city.2 <- KMPL.city.2 # Add a new column called KMPL.city.2
tail(names(Cars93.metric))
```
...
manufacturer.combined <- mapvalues(manufacturer, c("Chevrolet", "Pontiac", "Buick"), rep("GM", 3))  # Map Chevrolet, Pontiac and Buick to GM
...
- Let's pretend that in the class survey 'Program' was coded as an integer with 1 = HCPM, 2 = MISM, 3 = PPM
...
```{r}
survey <- transform(survey, Program = as.factor(mapvalues(Program, c(1, 2, 3), c("HCPM", "MISM", "PPM"))))
head(survey)
```
...
my.vector.1 
typeof(my.vector.1)  # All the elements are now a string!
```
...
- Often times we want our code to have different effects depending on the features of the input
