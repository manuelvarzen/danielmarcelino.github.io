...
\begin{itemize}
 \item Sampling Error \\
 \low{This is the one source of error that pollsters do report, and it captures the error associated with only measuring opinions in a random sample of the population, as apposed to among all voters.}
 \item Reference 2 \\
 \low{by Author}
 \item Reference 3 \\
 \low{by Author 3}
